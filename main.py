...
def get_news(topic):
    ...
    try:
        ...
        if response.status_code==200:
            ...
            news_json=json.loads(news) # type: ignore
...
class AssistantManager:
    ...
    assistant_id="asst_AOkR3Dda7ymbVslH8wXrpaDj"
    ...
    def wait_for_completed(self):
        if self.thread and self.run:
            while True:
                time.sleep(5)
                run_status=self.client.beta.thread.run.retrieve(
                    thread_id=self.thread.id, run_id=self.run.id
                )
                print(f"RUN STATUS:: {run_status.model_dump_json(indent=e)}")
                
                if run_status.status=="completion":
   ...
   with st.form(key="user_input_form"):
       ...
       if submit_button:
            manager.create_assistant(
               ...
               instructions= "You are the personal article summarizer Assistant who knows hoe to take a list of article's titles and description and then write a short summary of all the news articles", 
               tools=[
                   {
                    "type": "function",
                    "function":{
                        "name": "get_news",
                        "description": "Get the lost of articles/news for the given topic",
                        "parameters": {
                            "type":"object",
                            "properties":{
                                "topic":{
            ...
            # wait for the completions and process messages 
